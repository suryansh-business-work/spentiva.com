...
const MonthlyDetailsDialog: React.FC<MonthlyDetailsDialogProps> = ({
  open, onClose, selectedYear, monthlyExpenses, monthlyIncome, monthlyTransactions, currency,
}) => {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth
      ...
      <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', pb: 1 }}>
        <Typography variant="h6" fontWeight={600}>Monthly Details ({selectedYear})</Typography>
          ...
          <Table size="small">
            ...
            <TableBody>
              ...
              <TableRow sx={{ bgcolor: isDarkMode ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.03)' }}>
                ...
                <TableCell align="right" sx={{ fontWeight: 600 }}>{formatAmount(totalIncome, currency)}</TableCell>
                <TableCell align="right" sx={{ fontWeight: 600 }}>{totalTxn}</TableCell>
              </TableRow>
            </TableBody>
          </Table>
        </TableContainer>
